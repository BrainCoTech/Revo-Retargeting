...
protected:
    ...
    /// @brief Expand a path that may start with ~ to the full home directory path.
    std::string ExpandPath(const std::string& p_Path);
    
    /// @brief Attempt to load a calibration .mcal file for the given glove from the calibration directory.
