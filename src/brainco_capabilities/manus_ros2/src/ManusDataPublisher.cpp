...
std::string DefaultCalibrationDirectory()
{
    try
    {
        ...
        if (source_directory)
        {
            return source_directory->string();
        }
        return (package_share / "calibrations").string();
    }
    catch (const std::exception&)
    {
        return "~/Documents/manus-calibrations";
    }
}
}  // namespace
...
ManusDataPublisher::ManusDataPublisher() : Node("manus_data_publisher")
{
    ...
    const auto publish_period = std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::duration<double>(1.0 / m_PublishRateHz));
    m_PublishTimer = create_wall_timer(
        publish_period,
        [this] { PublishCallback(); });
    RCLCPP_INFO(
        get_logger(),
        "MANUS data publish rate: %.1f Hz",
        m_PublishRateHz);
...
void ManusDataPublisher::PublishCallback()
{
    ...
    for (size_t i = 0; i < m_Landscape->gloveDevices.gloveCount; i++)
    {
        ...
        {
            TryLoadCalibrationForGlove(t_Msg.glove_id, m_Landscape->gloveDevices.gloves[i]);
            m_CalibratedGloves.insert(t_Msg.glove_id);
        }
...
std::string ManusDataPublisher::ExpandPath(const std::string& p_Path)
{
    if (!p_Path.empty() && p_Path[0] == '~')
    {
        const char* t_Home = std::getenv("HOME");
        if (t_Home != nullptr)
        {
            return std::string(t_Home) + p_Path.substr(1);
        }
    }
    return p_Path;
}

bool ManusDataPublisher::TryLoadCalibrationForGlove(uint32_t p_GloveId, const GloveLandscapeData& p_Glove)
{
    const std::filesystem::path t_Dir = manus_ros2::ExpandUserPath(m_CalibrationDirectory);
    if (!std::filesystem::exists(t_Dir) || !std::filesystem::is_directory(t_Dir))
    {
        ClientLog::error("Calibration directory does not exist: {}", t_Dir.string().c_str());
        return false;
    }

    const std::string t_SideStr = manus_ros2::SideToSlug(p_Glove.side);
    const std::string t_FamilyStr = manus_ros2::DeviceFamilyToSlug(p_Glove.familyType);
    const std::filesystem::path t_ExpectedFile =
        manus_ros2::DefaultCalibrationPath(t_Dir, p_Glove.familyType, p_Glove.side);
    const std::optional<std::filesystem::path> t_SelectedFile =
        manus_ros2::FindCalibrationFile(t_Dir, p_Glove.familyType, p_Glove.side);
    if (!t_SelectedFile)
    {
        ClientLog::error(
            "Calibration file not found for {} {} glove (ID: {}). Expected exact file: {}",
            t_FamilyStr.c_str(), t_SideStr.c_str(), p_GloveId, t_ExpectedFile.string().c_str());
        return false;
    }

    // Read the calibration file as binary
    std::ifstream t_FileStream(*t_SelectedFile, std::ios::binary | std::ios::ate);
    if (!t_FileStream.is_open())
    {
        ClientLog::error("Failed to open calibration file: {}", t_SelectedFile->string().c_str());
        return false;
    }

    const std::streamsize t_Size = t_FileStream.tellg();
    t_FileStream.seekg(0, std::ios::beg);

    std::vector<unsigned char> t_Buffer(t_Size);
    if (!t_FileStream.read(reinterpret_cast<char*>(t_Buffer.data()), t_Size))
    {
        ClientLog::error("Failed to read calibration file: {}", t_SelectedFile->string().c_str());
        return false;
    }

    SetGloveCalibrationReturnCode t_CalibResult;
    const SDKReturnCode t_SdkResult = CoreSdk_SetGloveCalibration(
        p_GloveId, t_Buffer.data(), static_cast<uint32_t>(t_Size), &t_CalibResult);

    if (t_SdkResult != SDKReturnCode::SDKReturnCode_Success ||
        t_CalibResult != SetGloveCalibrationReturnCode_Success)
    {
        ClientLog::error(
            "Failed to set calibration for glove {} from file {}. SDK result: {}, Calibration result: {}",
            p_GloveId, t_SelectedFile->string().c_str(),
            static_cast<int32_t>(t_SdkResult), static_cast<int32_t>(t_CalibResult));
        return false;
    }
