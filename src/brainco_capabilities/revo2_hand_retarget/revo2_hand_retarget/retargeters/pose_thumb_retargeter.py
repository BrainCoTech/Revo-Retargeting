...
from revo2_hand_retarget.retargeters.dex_retargeter import (
    DexRetargeter,
...
CANONICAL_LANDMARK_SCALE = 1.13
FOUR_FINGER_NAMES = ("index", "middle", "ring", "little")
LEGACY_ERGONOMICS_FINGER_NAMES = {
    "index": "Index",
    "middle": "Middle",
    "ring": "Ring",
    "little": "Pinky",
}
LEGACY_STRETCH_WEIGHTS = {"MCP": 0.50, "PIP": 0.35, "DIP": 0.15}
...
def _apply_ema_to_landmark_targets(previous_targets, current_targets, ema_prev, ema_cur):
    filtered_targets = {}
...
def _four_finger_targets_from_joints(joint_positions):
    """Read source-neutral aggregate finger flexion angles in radians."""
    ...
    for finger_index, finger_name in enumerate(FOUR_FINGER_NAMES):
        key = f"{finger_name}_flexion"
        if key not in joint_positions:
            continue
        any_value = True
        targets[finger_index] = float(
            np.clip(float(joint_positions[key]), 0.0, FOUR_FINGER_MAX_RAD)
        )

    return targets if any_value else None


def _legacy_ergonomics_to_joint_positions(ergonomics):
    """Translate the pre-HandKinematics algorithm API at one compatibility edge."""
    if not ergonomics:
        return None
    result = {}
    for finger in FOUR_FINGER_NAMES:
        legacy_finger = LEGACY_ERGONOMICS_FINGER_NAMES[finger]
        weighted_degrees = 0.0
        found = False
        for joint, weight in LEGACY_STRETCH_WEIGHTS.items():
            key = f"{legacy_finger}{joint}Stretch"
            if key not in ergonomics:
                continue
            found = True
            weighted_degrees += weight * max(0.0, float(ergonomics[key]))
        if found:
            result[f"{finger}_flexion"] = float(
                np.clip(np.radians(weighted_degrees), 0.0, FOUR_FINGER_MAX_RAD)
            )
...
class PoseThumbRetargeter(BaseRetargeter):
    ...
    def __init__(self, config_file_path: Path, enabled_sides=None):
        ...
        try:
            ...
            self._dex = None
            logger.debug(
                "DexRetargeter fallback unavailable; canonical joint mapping "
                "remains active when aggregate flexion is available: %s",
                exc,
            )
    ...
    def _build_targets(self, finger_tip_pos):
        return {
            "thumb": np.asarray(finger_tip_pos[0], dtype=float) * CANONICAL_LANDMARK_SCALE,
            "index": np.asarray(finger_tip_pos[1], dtype=float) * CANONICAL_LANDMARK_SCALE,
            "middle": np.asarray(finger_tip_pos[2], dtype=float) * CANONICAL_LANDMARK_SCALE,
            "ring": np.asarray(finger_tip_pos[3], dtype=float) * CANONICAL_LANDMARK_SCALE,
            "pinky": np.asarray(finger_tip_pos[4], dtype=float) * CANONICAL_LANDMARK_SCALE,
        }
    ...
    def _build_thumb_target(transformed, pip_pos, thumb_reach_scale):
        ...
        if pip_pos is not None and center_4 is not None:
            pip_arr = np.asarray(pip_pos, dtype=float) * CANONICAL_LANDMARK_SCALE
    ...
    def retarget_side(
        ...
        **kwargs,
        ...
        four = _four_finger_targets_from_joints(joint_positions)
        if four is not None:
            q_target[
        ...
        filtered = _apply_ema_to_landmark_targets(
    ...
    def retarget_process(
        ...
        if left_joint_positions is None:
            left_joint_positions = _legacy_ergonomics_to_joint_positions(left_ergonomics)
        if right_joint_positions is None:
            right_joint_positions = _legacy_ergonomics_to_joint_positions(right_ergonomics)
        left_q_target = (
            self.retarget_side(
                "left",
                left_finger_tip_pos,
