...
from revo2_hand_retarget.retargeters import RetargeterRegistry
...
_CONTROL_CONFIG_FIELDS = {
    ...
    "input_data_timeout": (
        ("input", "input_data_timeout"),
        ("input", "glove_data_timeout"),
        ("input_data_timeout",),
        ("glove_data_timeout",),
    ),
...
def _resolve_control_config_path(control_config):
    ...
    try:
        ...
        package_share = Path(get_package_share_directory("revo2_hand_retarget"))
        candidates.extend([
...
class Revo2HandRetargetNode(Node):
    """Retarget source-neutral hand kinematics to Revo2 joint targets."""
    ...
    def __init__(
        ...
        # Canonical input order expected by all retargeting algorithms.
        ...
        # Do not publish a target until a complete canonical hand frame arrives.
        self.left_data_ready = not self.enable_left
        self.right_data_ready = not self.enable_right
        self.input_data_timeout = float(max(0.0, input_data_timeout))
        self._last_input_data_time = {"left": None, "right": None}
        self._last_input_timeout_log_time = {"left": 0.0, "right": 0.0}
        self._input_msg_counts = {"left": 0, "right": 0}
        self._last_input_source = {"left": "", "right": ""}
        self._last_valid_tip_count_by_side = {"left": 0, "right": 0}
        self._last_missing_tips_by_side = {
            "left": list(self.finger_tip_names),
            "right": list(self.finger_tip_names),
        }
        self._last_input_status_log_ts = 0.0
        ...
        self.hand_input_subscriptions = []
        if self.enable_left:
            self.hand_input_subscriptions.append(
                self.create_subscription(
                    HandKinematics,
                    "/hand_kinematics/left",
                    lambda msg: self.hand_kinematics_callback(msg, "left"),
                    20,
                )
            )
        if self.enable_right:
            self.hand_input_subscriptions.append(
                self.create_subscription(
                    ...
                    lambda msg: self.hand_kinematics_callback(msg, "right"),
                    20,
                )
            )
    ...
    def _mark_hand_data_ready(self, hand: str):
        self._last_input_data_time[hand] = time.monotonic()
        if hand == "left" and not self.left_data_ready:
            self.left_data_ready = True
            logger.info("检测到左手 HandKinematics 有效数据，开始发送左手控制。")
        elif hand == "right" and not self.right_data_ready:
            self.right_data_ready = True
            logger.info("检测到右手 HandKinematics 有效数据，开始发送右手控制。")

    def _expire_stale_input_data(self):
        if self.input_data_timeout <= 0.0:
            return
        now = time.monotonic()
        for side, enabled in (("left", self.enable_left), ("right", self.enable_right)):
            if not enabled:
                continue
            last_time = self._last_input_data_time[side]
            ready = self.left_data_ready if side == "left" else self.right_data_ready
            if not ready or last_time is None or now - last_time <= self.input_data_timeout:
                continue
            if side == "left":
                self.left_data_ready = False
            else:
                self.right_data_ready = False
            if now - self._last_input_timeout_log_time[side] >= 1.0:
                self._last_input_timeout_log_time[side] = now
                logger.warning(
                    "%s hand input timed out after %.3f s; target publishing paused.",
                    side,
                    self.input_data_timeout,
                )

    def _record_input_status(self, msg: HandKinematics, side: str):
        self._input_msg_counts[side] += 1
        self._last_input_source[side] = str(msg.source)
        now = time.time()
        total_count = sum(self._input_msg_counts.values())
        if total_count == 1 or now - self._last_input_status_log_ts >= 5.0:
            self._last_input_status_log_ts = now
            self.get_logger().info(
                "Canonical hand input: "
                f"side={side} source={msg.source or '(unknown)'} "
                f"joints={len(msg.joint_names)} landmarks={len(msg.landmark_names)} "
                f"counts={self._input_msg_counts}"
            )
    ...
    def _log_waiting_for_hand_data(self):
        ...
        self.get_logger().warning(
            "Waiting for complete HandKinematics before publishing target. "
            f"enabled=(left:{self.enable_left} right:{self.enable_right}), "
            f"ready=(left:{self.left_data_ready} right:{self.right_data_ready}), "
            f"counts={self._input_msg_counts}, "
            f"sources={self._last_input_source}, "
            f"missing_tips={self._last_missing_tips_by_side}"
        )
    ...
    def hand_kinematics_callback(self, msg: HandKinematics, expected_side: str):
        """Validate and atomically replace one side's canonical input frame."""
        t0 = time.time() if self.enable_timing_print else None
        expected_value = HandKinematics.LEFT if expected_side == "left" else HandKinematics.RIGHT
        if int(msg.side) != expected_value:
            logger.warning(
                "Dropping HandKinematics on %s topic with side=%s",
                expected_side,
                msg.side,
            )
            return
        if not msg.header.frame_id:
            logger.warning("Dropping %s HandKinematics with an empty frame_id", expected_side)
            return
        if len(msg.joint_names) != len(msg.joint_positions_rad):
            logger.warning("Dropping %s HandKinematics with mismatched joint arrays", expected_side)
            return
        if len(msg.landmark_names) != len(msg.landmarks_m):
            logger.warning("Dropping %s HandKinematics with mismatched landmark arrays", expected_side)
            return

        joint_positions = {}
        for name, value in zip(msg.joint_names, msg.joint_positions_rad):
            value = float(value)
        ...
        for name, point in zip(msg.landmark_names, msg.landmarks_m):
            value = np.asarray([point.x, point.y, point.z], dtype=float)
            if name and np.all(np.isfinite(value)):
                landmarks[str(name)] = value

        missing = [name for name in self.finger_tip_names if name not in landmarks]
        self._last_valid_tip_count_by_side[expected_side] = len(self.finger_tip_names) - len(missing)
        self._last_missing_tips_by_side[expected_side] = missing
        if missing:
            logger.warning(
                "Dropping incomplete %s HandKinematics; missing %s",
                expected_side,
                missing,
            )
            return

        finger_tips = [landmarks[name].tolist() for name in self.finger_tip_names]
        if expected_side == "left":
            self.left_finger_tip_pos = finger_tips
            self.left_thumb_dip_pos = landmarks.get("thumb_dip")
            self.left_thumb_pip_pos = landmarks.get("thumb_pip")
            self.left_joint_positions = joint_positions
        else:
            self.right_finger_tip_pos = finger_tips
            self.right_thumb_dip_pos = landmarks.get("thumb_dip")
            self.right_thumb_pip_pos = landmarks.get("thumb_pip")
            self.right_joint_positions = joint_positions

        self._record_input_status(msg, expected_side)
        self._mark_hand_data_ready(expected_side)
        if self.enable_timing_print:
            dt = time.time() - t0
            if dt > 0.005:
                print(f"[TIMING] hand_kinematics_callback: {dt*1000:.1f} ms")
    ...
    def retarget_callback(self):
        ...
        try:
            ...
            [left_target, right_target] = self.hand_retargeting.retarget_process(
                ...
                left_joint_positions=self.left_joint_positions,
                right_joint_positions=self.right_joint_positions,
            )
    ...
    def _wait_for_hand_data_ready(self, hand: str, timeout_sec: float = 20.0):
        ...
        while time.time() < deadline:
            if self._is_hand_data_ready(hand):
                return
            time.sleep(0.05)
        raise TimeoutError(f"{hand} 手在 {timeout_sec:.1f}s 内未收到有效 HandKinematics")
    ...
    def _sample_hand_pose_median(
        ...
        if len(thumb_tip_samples) < 3:
            raise RuntimeError(f"{hand} 手标定采样不足，请保持手势稳定并确保输入数据正常")
...
class Revo2HandRetargeter:
    ...
    def __init__(
        ...
        # 创建单一节点
        self.node = Revo2HandRetargetNode(
            hand_mode=hand_mode,
            skip_calibration=skip_calibration,
            calibration_file=calibration_file,
            use_default_pose_thumb_calibration=use_default_pose_thumb_calibration,
...
def parse_cli_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Source-neutral Revo2 hand-retarget node launcher."
    )
    parser.add_argument(
        "--control-config",
        "--control_config",
    ...
    parser.add_argument(
        ...
        help="Calibration file path for saving/loading thumb calibration data. "
             "Defaults to ~/.revo2_hand_retarget/thumb_calibration_<hand_mode>.json; "
    ...
    parser.add_argument(
        "--input-data-timeout",
        "--glove-data-timeout",
        dest="input_data_timeout",
        type=float,
        default=0.3,
        help="Seconds before stale hand input pauses target publication. 0 disables the timeout.",
    )
    parser.add_argument(
    ...
    if args.calibration_file is None:
        default_path = (
            Path.home()
            / ".revo2_hand_retarget"
            / f"thumb_calibration_{args.hand_mode}.json"
        )
        legacy_path = Path.home() / f".manus_revo2/thumb_calibration_{args.hand_mode}.json"
        args.calibration_file = str(legacy_path if legacy_path.is_file() else default_path)
...
def main(argv=None):
    try:
        ...
        retargeter = Revo2HandRetargeter(
