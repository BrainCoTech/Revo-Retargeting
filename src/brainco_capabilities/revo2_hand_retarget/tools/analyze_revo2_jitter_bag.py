...
def load_bag(args):
    ...
    input_topic = args.input_topic or f"/hand_kinematics/{args.side}"
...
def print_target_jumps(data, args):
    ...
    input_gaps = find_gap_events(data["stamps"].get(data["topics"]["input"], []), args.gap_ms)
...
def print_recommendation(data, args):
    print("\n== Quick Read ==")
    raw_gap_counts = []
    for topic in (data["topics"]["raw_angles"], data["topics"]["raw_positions"], data["topics"]["input"]):
    ...
    if max(raw_gap_counts or [0]) > 0 and big_target_jumps > 0:
        ...
        print(
            ...
            "Check retarget parameters and normalized hand-input noise."
        )
    else:
        print(
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Offline jitter analysis for Revo2 hand-teleoperation rosbags."
    )
    ...
    parser.add_argument("--input-topic", "--glove-topic", dest="input_topic", default="")
