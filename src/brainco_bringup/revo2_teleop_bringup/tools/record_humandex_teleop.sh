...
Example:
  ./src/brainco_bringup/revo2_teleop_bringup/tools/record_humandex_teleop.sh \
...
if [[ -f "${RETARGET_SETUP}" ]]; then
  ...
  echo "       Build/source it before recording HandKinematics topics." >&2
fi
...

{
  ...
  for hand in "${SIDES[@]}"; do
    ...
    input_topic="/hand_kinematics/${hand}"
    echo "python ${RETARGET_WS}/src/brainco_capabilities/revo2_hand_retarget/tools/analyze_revo2_jitter_bag.py ${BAG_DIR} --storage-id ${STORAGE_ID} --side ${hand} --input-topic ${input_topic} --raw-angles-topic ${raw_topic} --raw-positions-topic /humandex_eef_pose"
  done
